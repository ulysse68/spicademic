...
class file_status_so extends so_sql{
	
	var $spicademic_ref_file_status = 'spicademic_ref_file_status';
	var $spicademic_transition_status = 'spicademic_ref_file_status_transition';
	 ...
	 */
		$tab_search=array();
		foreach((array)$this->so_file_status->db_data_cols as $id=>$value){
			$tab_search[$id]=$search;
		}
		return $tab_search;
	}

	function add_update_file_status($info){
	/**
	 ...
	 */
		...
		if(is_array($info)){
			...
			unset($info['nm']);
			unset($info['msg']);
			$this->so_file_status->data = $info;
			if(isset($this->so_file_status->data['status_id'])){
				// Existant
				$this->so_file_status->data['status_modified']=time();
				$this->so_file_status->data['status_modifier']=$GLOBALS['egw_info']['user']['account_id'];
				$this->so_file_status->update($this->so_file_status->data,true);

				$infoTransition['status_id'] = $this->so_file_status->data['status_id'];
				...
				// Nouveau
				$this->so_file_status->data['status_id'] = '';
				$this->so_file_status->data['status_created']=time();
				$this->so_file_status->data['status_creator']=$GLOBALS['egw_info']['user']['account_id'];
				$this->so_file_status->save();
