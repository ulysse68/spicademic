...
require_once(EGW_INCLUDE_ROOT. '/spicademic/inc/class.file_status_bo.inc.php');
require_once(EGW_INCLUDE_ROOT. '/spicademic/inc/class.acl_spicademic.inc.php');

class file_status_ui extends file_status_bo{
	 ...
	 *
	 */
	function file_status_ui(){
		parent::file_status_bo();
	 ...
	 */
		...
		if(isset($content['nm']['rows']['delete'])){
			...
			if($this->so_file_status->delete($id)){
		...
		if (!is_array($content['nm']))
		{
			...
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spicademic.file_status_bo.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
		...
		$tpl = new etemplate('spicademic.file_status.index');
		$content['nm']['header_right'] = 'spicademic.file_status.index.right';
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Status management');
		$tpl->read('spicademic.file_status.index');
		$tpl->exec('spicademic.file_status_ui.index', $content, $sel_options, $readonlys, array('nm' => $content['nm']));
	}
	 ...
	 */
		...
		if(is_array($content)){
			...
			switch($button){
				case 'save':
				case 'apply':
					$msg = $this->add_update_file_status($content);
					if($button=='save'){
			...
			$id = $this->so_file_status->data['status_id'];
		...
		$tpl->exec('spicademic.file_status_ui.edit', $content, $sel_options, $readonlys, $content,2);
	}
}
?>