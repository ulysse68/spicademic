...
require_once(EGW_INCLUDE_ROOT. '/spicademic/inc/class.file_type_bo.inc.php');
require_once(EGW_INCLUDE_ROOT. '/spicademic/inc/class.acl_spicademic.inc.php');

class file_type_ui extends file_type_bo{
	 ...
	 */
		...
		if(isset($content['nm']['rows']['delete'])){
			...
			if($this->so_file_type->delete($id)){
		...
		{
			$default_cols='file_type_id,file_type_label,file_type_description,file_type_active,file_type_order';
			$content['nm'] = array(                           // I = value set by the app, 0 = value on return / output
				'get_rows'       	=>	'spicademic.file_type_bo.get_rows',	// I  method/callback to request the data for the rows eg. 'notes.bo.get_rows'
				...
				'order'          	=>	'file_type_id',	// IO name of the column to sort after (optional for the sortheaders)
		...
		$tpl = new etemplate('spicademic.file_type.index');
		$content['nm']['header_right'] = 'spicademic.file_type.index.right';
		$GLOBALS['egw_info']['flags']['app_header'] = lang('Status management');
		$tpl->read('spicademic.file_type.index');
		$tpl->exec('spicademic.file_type_ui.index', $content, $sel_options, $readonlys, array('nm' => $content['nm']));
	}
	 ...
	 */
		...
		if(is_array($content)){
			...
			switch($button){
				case 'save':
				case 'apply':
					$msg = $this->add_update_file_type($content);
					if($button=='save'){
			...
			$id = $this->so_file_type->data['file_type_id'];
		...
		// Listes
		$sel_options = array(
			'file_type_parent' => $this->get_parents($content['file_type_id']),
			'file_type_access' => $this->get_access(),
		);
		
		$tpl = new etemplate('spicademic.file_type.edit');
		$tpl->read('spicademic.file_type.edit');
		$tpl->exec('spicademic.file_type_ui.edit', $content, $sel_options, $readonlys, $content,2);
	}
}
?>