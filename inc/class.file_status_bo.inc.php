...
require_once(EGW_INCLUDE_ROOT. '/spicademic/inc/class.file_status_so.inc.php');	

class file_status_bo extends file_status_so{
	
	/**
	 * Constructeur
	 *
	 */
	function file_status_bo(){
		parent::file_status_so();
	}
	 ...
	 * @return array
	 */
		$info = $this->so_file_status->read($id);
	 ...
	 */
		...
		$rows = $this->so_file_status->search($search,false,$order,'',$wildcard,false,$op,$start,$query['col_filter']);
		if(!$rows){
			$rows = array();
		}
		...
		return $this->so_file_status->total;	
    }
	 ...
	 */
		$retour = array();
		$info = $this->so_file_status->search(array('status_active' => true),false,'status_label');
		$i = 0;
